...
const printData = computed(() => {
  ...
  const colEnumList = colSetting.filter(item => item.enum)
...
const handlePrint = () => {
  printJS({
    ...
    header: `<div style="display: flex;flex-direction: column;text-align: center"><h2>${props.title}</h2></div>`,
...
<template>
  <!-- 查询表单 -->
  <SearchForm v-show="isShowSearch" :search="search" :reset="reset" :search-param="searchParam" :columns="searchColumns" />
  <div class="card table">
    <!-- 表格头部 操作按钮 -->
    <div class="table-header">
      <div class="header-button-lf">
        <slot name="tableHeader" :selected-list-ids="selectedListIds" :select-list="selectedList" :is-selected="isSelected" />
      </div>
