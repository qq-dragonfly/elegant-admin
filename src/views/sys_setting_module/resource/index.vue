		...
		>
			...
			<template #tableHeader>
				<Auth :value="['add:resource:btn']">
					<el-button type="primary" :icon="CirclePlus" @click="openDialog('add')">新增资源</el-button>
				</Auth>
			</template>
			<!-- Expand -->
...
import { getSysResourceListApi, addSysResourceApi, editSysResourceApi, delSysResourceApi } from '@/api/modules/resource';
// 组件引入
...
const columns: ColumnProps[] = [
	...
	{ prop: 'label', label: '资源名称', minWidth: 120, search: { el: 'input' } },
	{
		prop: 'openStatus',
		label: '资源状态',
...
const deleteAccount = async (params: any) => {
	await useHandleData(delSysResourceApi, { id: params.id }, `删除【${params.label}】`);
...
const openDialog = (type: string, rowData: Partial<any> = {}) => {
	let params = {
		...
		rowData: { ...rowData },
		isView: type === 'show',
		api: type === 'add' ? addSysResourceApi : type === 'edit' ? editSysResourceApi : addSysResourceApi,
