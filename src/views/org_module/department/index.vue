	...
	<div class="table-box">
		...
		>
			...
			<template #tableHeader>
				<Auth :value="['add:dep:btn']">
					<el-button type="primary" :icon="CirclePlus" @click="openDialog('add')">新增科室</el-button>
				</Auth>
			</template>
			<!-- Expand -->
...
<script setup lang="tsx" name="OrgDepartment">
...
const getTableList = (params: any) => {
	...
	return getDepartmentListApi(newParams);
};

// 表格配置项
const columns: ColumnProps[] = [
	{ type: 'index', label: '#', width: 60 },
	{ prop: 'name', label: '科室名称', minWidth: 120, search: { el: 'input' } },
	{
		prop: 'openStatus',
		label: '状态',
		...
		width: 100,
		search: { el: 'tree-select', key: 'openStatus', props: { filterable: true } }, //filterable 开启模糊搜索
	...
	{ prop: 'sort', label: '排序值', minWidth: 100 },
	{
		...
		// enum: getDictCodeApi,
		// 字典请求携带参数
		enum: () => getDictTypeApi({ type: 'STANDARD_DEPT' }),
		minWidth: 140,
		search: {
			el: 'cascader',
			key: 'standardDeptId',
			props: { filterable: true, checkStrictly: true, emitPath: false } //filterable 开启模糊搜索
		},
...
const deleteAccount = async (params: any) => {
	await useHandleData(delDepartmentApi, { id: params.id }, `删除【${params.name}】`);
...
const openDialog = (type: string, rowData: Partial<any> = {}) => {
	let params = {
		...
		rowData: { ...rowData },
		isView: type === 'show',
		api: type === 'add' ? addDepartmentApi : type === 'edit' ? editDepartmentApi : addDepartmentApi,
